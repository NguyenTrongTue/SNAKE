#include "snake.hpp"
#include "sdl_check.hpp"

Snake::Snake() // truy cap lop class
{
    auto res = SDL_Init(SDL_INIT_EVERYTHING);
    SDL_CHECK(res == 0, "SDL_Init");
    SDL_CreateWindowAndRenderer(Width, Height, SDL_WINDOW_BORDERLESS, &window, &renderer);//ham khoi tao cua so
    SDL_CHECK(window, "SDL_CreateWindowAndRenderer");
    SDL_CHECK(renderer, "SDL_CreateWindowAndRenderer");
    SDL_SetWindowPosition(window, 100, 20);
    auto surface = SDL_LoadBMP("sprites.bmp"); // ham chen anh
    SDL_CHECK(surface, "SDL_LoadBMP(\"sprites.bmp\")");
    sprites = SDL_CreateTextureFromSurface(renderer, surface);
    SDL_CHECK(sprites, "SDL_CreateTextureFromSurface");
    SDL_FreeSurface(surface);

// hàm định hướng di chuyển cho rắn
    segmentsList.push_back(std::make_pair(5, 5));
    segmentsList.push_back(std::make_pair(5, 6));
    segmentsList.push_back(std::make_pair(4, 6));
// ham khoi tao ngau nhien cherry
    generateFruit();
}


 // ham tao ra cherry cho ran an

void Snake::generateFruit()
{
    auto done = false;
    fruitX = rand() % (Width / 64);//64
    fruitY = rand() % (Height / 64);//64
    done = true;
    for ( auto &segment: segmentsList)
    {
      if (fruitX == segment.first && fruitY == segment.second)
      {
        done = false;
        break;
      }
    }
}

// hàm hủy snake giải phóng bộ nhớ

Snake::~Snake()
{
  SDL_DestroyRenderer(renderer);
  SDL_DestroyWindow(window);
  SDL_Quit();
}
// ham xu li phim di chuyen nhan tu ban phim nguoi choi
int Snake::exec()
{
  auto oldTick = SDL_GetTicks();
  for (auto done = false; !done;)
  {
    SDL_Event e;
    if (SDL_PollEvent(&e))
    {
      switch (e.type)
      {
      case SDL_KEYDOWN:
        {
          switch (e.key.keysym.sym)
          {
          case SDLK_UP:
            dx = 0;
            dy = -1;
            break;
          case SDLK_RIGHT:
            dx = 1;
            dy = 0;
            break;
          case SDLK_DOWN:
            dx = 0;
            dy = 1;
            break;
          case SDLK_LEFT:
            dx = -1;
            dy = 0;
            break;
          }
        }
        break;
        case SDL_QUIT:
        done = true;
        break;
      }
    }
    // ve nen cho reo choi
    SDL_SetRenderDrawColor(renderer, 0, 255, 0, 255);
    SDL_RenderClear(renderer); // xoa toan bo hinh nen cu di
    auto currentTick = SDL_GetTicks();
    for (auto t = oldTick; t < currentTick; ++t)
      if (!tick())
        return 1;
    oldTick = currentTick;
    draw();
    SDL_RenderPresent(renderer);
  }
  return 0;
}


// ham kiem tra va cham
bool Snake::tick()
{
  if (ticks++ % 250 == 0) // xu li do nhanh cham cho snake khi di chuyen
  {
    // khoi tao bien p cho danh sach phan doan than ran
    auto p = segmentsList.front();
    p.first += dx; // chieu rong cua tung phan doan
    p.second += dy; // chieu dai cua tung phan doan
    if (p.first < 0 || p.first >= Width / 64 ||
        p.second < 0 || p.second >= Height / 64)//64
      return false;
    for (const auto &segment: segmentsList)
      if (p == segment)
        return false;
    segmentsList.push_front(p);
    if (p.first != fruitX || p.second != fruitY)
      segmentsList.pop_back();
    else
      generateFruit();
  }
  return true;
}

void Snake::draw()
{
    // do lon cua phan than dau cherry trong game
  SDL_Rect src;
  src.x = 0;
  src.y = 0;
  src.w = 64;//64
  src.h = 64;//64
  //----------------
  SDL_Rect dest;// đích đến của bộ phận
  dest.w = 64;//64
  dest.h = 64;//64
  //mang 2 chieu danh sach hướng quay và góc di chuyển
  int ds[4 ][3] = {
    { -1, 0, 0 }, // sang trái
    { 0, -1, 90 }, // lên
    { 1, 0, 180 }, // sang phải
    { 0, 1, -90 }, // xuống
  };
  for (auto iter = std::begin(segmentsList); iter != std::end(segmentsList); ++iter)
  {
    float direction = 0;// khoi tao huong
    const auto &segment = *iter; // khởi tạo bộ phận
    if (iter == std::begin(segmentsList))
    {
      if (iter->first + dx == fruitX && iter->second + dy == fruitY)
        src.x = HeadOpenMouth * 64;//64
      else
        src.x = Head * 64;//64
      if (iter + 1 != std::end(segmentsList))
      {
        const auto &nextSegment = *(iter + 1);
        for (const auto &d: ds)
        {
          if (segment.first + d[0] == nextSegment.first && segment.second + d[1] == nextSegment.second)
          {
            direction = d[2];
            break;
          }
        }
      }
    }
    else if (iter + 1 == std::end(segmentsList))
    {
      src.x = Tail * 64;//64
      const auto &prevSegment = *(iter - 1); // phan doan truoc
      for (const auto &d: ds)
      {
        if (segment.first == prevSegment.first + d[0] && segment.second == prevSegment.second + d[1])
        {
          direction = d[2];
          break;
        }
      }
    }
    else
    {
      const auto &nextSegment = *(iter + 1);
      const auto &prevSegment = *(iter - 1);
      //su li khau quay ran
      if (nextSegment.first == prevSegment.first) // phan doan truoc bang phan doan sau thi tang phan than va goc quay la 90
      {
        src.x = Straight * 64;//64
        direction = 90;
      }
      else if (nextSegment.second == prevSegment.second)
      {
        src.x = Straight * 64;//64
        direction = 0;
      }
      else
      {
        src.x = Turn * 64;//64
        bool up = false;
        if (segment.first == nextSegment.first && segment.second - 1 == nextSegment.second)
          up = true;
        if (segment.first == prevSegment.first && segment.second - 1 == prevSegment.second)
          up = true;
        bool right = false;
        if (segment.first + 1 == nextSegment.first && segment.second == nextSegment.second)
          right = true;
        if (segment.first + 1 == prevSegment.first && segment.second == prevSegment.second)
          right = true;
        bool down = false;
        if (segment.first == nextSegment.first && segment.second + 1 == nextSegment.second)
          down = true;
        if (segment.first == prevSegment.first && segment.second + 1 == prevSegment.second)
          down = true;
        bool left = false;
        if (segment.first - 1 == nextSegment.first && segment.second == nextSegment.second)
          left = true;
        if (segment.first - 1 == prevSegment.first && segment.second == prevSegment.second)
          left = true;
          // sử lí góc quay cho snake giong nhu quay mot doan thang thanh hinh vuong
        if (up && right)
          direction = 0;
        else if (right && down)
          direction = 90;
        else if (down && left)
          direction = 180;
        else if (left && up)
          direction = 270;
      }
    }
// xu li bo phan cua ran cho phan dich den
    dest.x = 64 * segment.first;//64
    dest.y = 64 * segment.second;//64
    SDL_RenderCopyEx(renderer, sprites, &src, &dest, direction, nullptr, SDL_FLIP_NONE);
  }
  src.x = Fruit * 64;//64
  dest.x = fruitX * 64;//64
  dest.y = fruitY * 64;//64
  SDL_RenderCopyEx(renderer, sprites, &src, &dest, 0, nullptr, SDL_FLIP_NONE);
}



